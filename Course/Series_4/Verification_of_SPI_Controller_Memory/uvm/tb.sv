...
class writeb_readb extends uvm_sequence#(transaction);
    ...
    virtual task body();
        repeat(10) begin  // write 10 transactions
            tr = transaction::type_id::create("tr");
            tr.addr_c.constraint_mode(1);
            tr.addr_c_err.constraint_mode(0);
            start_item(tr);
        ...
        repeat(10) begin  // read 10 transactions
            tr = transaction::type_id::create("tr");
            tr.addr_c.constraint_mode(1);
            tr.addr_c_err.constraint_mode(0);
            start_item(tr);
        ...
        if (tr.op == rstdut) begin
            ...
            if (tr.err == 1'b1) begin
                ...
                if (data_rd == tr.dout) begin
                    ...
                    `uvm_info("SCO", $sformatf("DATA FAILED  : addr:%0d, rdata:%0d, data_rd_arr:%0d", tr.addr, tr.dout, data_rd), UVM_NONE);
                end
            end
        end
        $display("------------------------------------------------------");
...
module tb;
    ...
    dut_top dut (.wr(vif.wr), .clk(vif.clk), .rst(vif.rst), .addr(vif.addr), .din(vif.din), .dout(vif.dout), .done(vif.done), .err(vif.err));
